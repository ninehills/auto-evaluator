...
class _WenxinClient(object):
    ...
    def _construct_message(self, prompt:str, history: List[Tuple[str, str]]) -> List[Any]:
        ...
        for human, ai in history:
            messages.append(dict(role="user", content=human))
            messages.append(dict(role="assistant", content=ai))
        messages.append(dict(role="user", content=prompt))
        return messages

    def completion_stream(self, prompt: str,
        ...
        r = requests.post(
            ...
            json=dict(messages=self._construct_message(prompt, history), stream=True),
    ...
    def completion(self, prompt:str, history: List[Tuple[str, str]], **params) -> str:
        """Call out to Wenxin's generate endpoint.
        ...
        r = requests.post(
            url=self.WENXIN_COMPLETION_URL,
            params={"access_token": self.grant_token()},
            json=dict(messages=self._construct_message(prompt, history)),
            timeout=self.request_timeout,
        )
        r.raise_for_status()
        response = r.json()
...
class ChatWenxin(BaseChatModel, _BaiduCommon):
    ...
    def _convert_messages_to_prompt(
        ...
        for message in messages[:-1]:
            ...
            if pair[0] is None:
                if message.type == "human":
                    ...
                    raise ValueError("It must be in the order of user, assistant.")
            else:
                if message.type == "ai":
        ...
        if not isinstance(messages[-1], HumanMessage):
            raise ValueError("The last message must be a human message.")
    ...
    def _generate(
        ...
        params: Dict[str, Any] = {"prompt": prompt, "history": history, **self._default_params}
    ...
    async def _agenerate(
        ...
        raise NotImplementedError("Async not implemented for Wenxin Chat Model.")


if __name__ == "__main__":
    #client = _WenxinClient("wenxin", api_key, secret_key, 60)
    #print("Check grant token")
    #print(client.grant_token())
    #print("Check completion")
    #print(client.completion("你是谁", [("你好", "你好啊！")]))
    #print("Check completion stream")
    #for data in client.completion_stream("你是谁", [("你好", "你好啊！")]):
    #    print(data)
    
    #print("Check Wenxin LLM")
    #llm = Wenxin(baidu_api_key=api_key, baidu_secret_key=secret_key)
    #print(llm("你好"))
    #for data in llm.stream("你好"):
    #    print(data)
    llm = ChatWenxin()
